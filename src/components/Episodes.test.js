...
test("display episodes as the prop is already updated", () => {
    const {rerender} = render(<Episodes episodes={[]}/>)

    expect(screen.queryByText(/chapter/i)).toBeNull();
    expect(screen.queryAllByAltText(/chapter/i)).toHaveLength(0);
    
    rerender(<Episodes episodes={episodeData}/> );
    
    expect(screen.getAllByText(/chapter/i))
    expect(screen.getAllByAltText(/chapter/i))
    expect(screen.getAllByText(/season/i))
    expect(screen.getAllByText(/episode/i))
})